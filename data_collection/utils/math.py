...
def apply_delta_pose(
    ...
    # TODO: Minu convert target_rot to euler_xyz and make yaw 0 and convert back to quat
    target_rot_euler = euler_xyz_from_quat(target_rot)
    # target_rot = quat_from_euler_xyz(target_rot_euler[0], target_rot_euler[1], torch.zeros_like(target_rot_euler[2]))
    target_rot = quat_from_euler_xyz(torch.zeros_like(target_rot_euler[0]), -torch.pi * torch.ones_like(target_rot_euler[1]), target_rot_euler[2])
